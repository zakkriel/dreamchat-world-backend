...
| Am I **merging** a migration? Then **apply it to production FIRST**, or production goes down the moment it deploys. | `ADR-P020` again, and this is the half that got skipped: the deploy does not apply migrations and the binary refuses to boot on drift, deliberately. On 2026-08-30 a merged migration took production to `502` for ~50 minutes while backend CI, the workspace gate, four PRs and both test suites stayed green — **none of them looks at production.** Release order: apply config → **apply migrations** → merge → **watch it boot** → exercise the path. `../harness/loose-ends.sh` now checks for this drift. |
