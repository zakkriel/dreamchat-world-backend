# G — Las Casas de Grelda, genesis under `world_model/4`

> **TEST-VALIDITY DISCLOSURE, read first.** In an earlier round of this same session I was assigned the
> Grelda **3-detallado** and **2-medio** files for the v1 stress test, and they are in my context. I
> did not open them this round, but I cannot unread them. So my run is **not a clean generative test**
> and its result should be discounted accordingly. Mitigation: I have deliberately excluded every
> specific I can identify as coming from those tiers — the three rings, the eleven Old Houses, the
> planters and cuttings, the Guild's membership/code/exam, Ordo's lost hearing, Perla's ration cut and
> notebook, Tomás's escape from a sealing, the register's missing months, and the named Sin Trato
> speaker. Where the pull to import one was strong I say so in §5. Everything below traces to the
> 400-word brief; §4 states each chain so you can audit it. Recommend re-running this world with an
> uncontaminated encoder.
...
**S1** pass — all seven extents reachable from `la subida` carry content: the climb has bowls and doorways; the Eighty-Three an agent; la Cuarenta a family and a new room; Ordo's house a master and a rod; the granary grain, a ledger and Perla; the square tents, Tomás and the Sin Trato; Cuesta Menor the district's houses. **S2** pass — every reachable agency wants something the player can act on: answer the house, satisfy Ordo, get an allotment changed, give Tomás an answer, take or refuse the door. **S3** pass — every name in any authored string resolves to an entity or is `excluded`; I removed "the city's other districts" for this reason. **S4** pass — both passages lead to authored extents; the Eighty-Three's door obstructs for a fictional reason (no pact). **S5** pass — hunger and emptiness both terminate; all three oppositions have reachable outcomes. **S6** pass — the Junta produced a granary, an office, a ledger and a law; the trade produced a collective, a master's office and a rod.

## 3. Validity

**O1**–**O11** pass: nine extents and two passages; agency on ten entities, each with `pursuing`; `hiding` on four; three oppositions; `bulk_class` on every non-agency `matter` entity (F4); both demanded substances suppliable — grain from the granary, people-noise emitted by presence; both accumulators have raisers and ordered ladders; one arrival; `excluded` present with five entries.

**R1**–**R13** pass. R3: every quantity is a class; numbers appear only in `exemplar` and in player-read prose (F1). R5: `las casas de la Cuesta` and `los Sin Trato` are never referenced individually — Tomás and la Cuarenta are separately authored, not subtracted (F5). R7: `holds` appears only on the granary, which has `holding`; the houses have `demand` without `holding` (F2). R8: `within` is a tree rooted at Grelda. R12: the disputed event has the Sin Trato flatly wrong against the apprentice. R13: every chain bottoms out in a quoted brief line.

**One contradiction in the contract, found by trying to satisfy it.** §4's facet table gates `within`
to `extent`, but D1 makes `within` the sole containment relation for *every* entity-in-entity pair. A
rod inside a house, a person inside a room and a ledger inside a granary all need `within` and none of
them has `extent`. Read strictly, R7 rejects almost every document the contract requires. I encoded
`within` as an ungated general key; if that reading is wrong, this document fails R7 fourteen times
and so will every other. Fix: move `within` out of the `extent` row.

## 4. Inference log — the ten that matter

| # | Inferred | Forced by | Generic I rejected |
|---|---|---|---|
| 1 | **The Eighty-Three is starving of people, not grain** | "a house alone and in silence goes out" + sixty years alone + it is still warm | that it is haunted, cursed, or hiding a body |
| 2 | **`emptiness` as a second accumulator beside hunger** | two of the three foods (fire-heat, people-noise) cannot be rationed, so the Junta's ledger cannot see this axis | one hunger track, grain only |
| 3 | **The knocking is a symptom, not a message** | "you do not know enough to understand" + houses have no words | a code with a decipherable text; a prophecy |
| 4 | **The granary, and grain as a stored, allotted substance** | Perla "decides how much grain each house gets" and a register records it | a market; a tax |
| 5 | **The Eighty-Three is on the smallest allotment on the hill** | it eats, it accepts nobody, and someone writes the number | a conspiracy to starve it |
| 6 | **`los tratantes` as a trade, not an institution** | "some people do this professionally" + master and apprentice | a licensed guild with a code, an exam and a monopoly |
| 7 | **Ordo has knocked there himself and been refused** | he is the best on the hill; the house is on his hill; he forbids the boy and will not say why | that he knows a secret about the house |
| 8 | **Perla's exposure is the ledger's own history** | she writes the record; the record goes back further than she does | that she falsified it |
| 9 | **Tomás leaves full bowls at other people's doors** | eleven years refused, wants an explanation, the bowl is courtesy a house notices | a protest movement, an occupation |
| 10 | **`green-eared` as a condition hindering the floor channel** | "you still can't hear the floor properly" is stated of the player | a skill number; a training montage |

## 5. The pull

**A tavern or a market.** The brief gives no gathering place, and every instinct wanted one. What it gives instead is **the bowl at every threshold** — a daily act performed at doors, in public, along a climb. The street *is* the social space, so `la subida` became the commons and the bowl the thing people are seen doing.

**A guard on the Eighty-Three's door.** Sixty years of refusals begs for someone posted there. But the brief's own physics makes a guard redundant and slightly absurd: a house that cannot be forced needs no one to stop you. What it gives instead is **Ordo's flat prohibition** — the only barrier is a person who will not explain, which is a far better door.

**A leader for the Sin Trato.** Eight hundred people in tents with a grievance wants a spokesperson, and I nearly wrote one. The brief instead hands me **Tomás**, whose eleven years and missing reason are more useful than any organiser: the Sin Trato stay a `magnitude` with an interest, and the way in is one man who walks to the foot of the hill and does not climb it.

## 6. Load-bearing set — shown before spend

By §2.1, inferred elements referenced elsewhere or appearing in law/accumulators/opposition/offices/excluded:

`emptiness` (accumulator, drives the house) · the Eighty-Three's `pursuing` and `hiding` (referenced by opposition and indicator) · **the allotment law** and `Alimentadora de Cuesta Menor` (office) · `el granero de la Junta` (supplies the demand) · `los tratantes` and `maestro tratante` (gate the emptiness indicator) · the Perla–Eighty-Three opposition · the houses' standing toward the apprentice · `condition: going out` · `condition: green-eared` · the four `excluded` entries I inferred.

Texture staying hidden: `la Cuarenta`, `la casa de Ordo`, `la subida`'s description, the day cycle, Ordo's and Tomás's `hiding`, all `seen_as` strings.
