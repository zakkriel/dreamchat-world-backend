...
| consumes | **Play Loop** (WE-7) | pre-action cue or post-perception trigger | `worldFirst` and `postCommittedCognition` use one `runCognition` engine. Unsaid speech stays out of the first trigger; only accepted holder-specific records enter the second. Decisions use the existing commit pipeline and cannot start a recursive dialogue cascade (`ADR-038`). |
