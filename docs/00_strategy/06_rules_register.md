...
| B-11 | **Event-driven cognition.** A holder's beliefs and appraisals update *only* on perception (witnessing, told, discovery-on-arrival, inference) — never on a free-running idle loop. Every belief-update carries a perceptual trigger and provenance (I-2; B-10 `linked_*`). Corollary: relationships are belief-perceptions, not canon (reinforces B-3/B-4/B-6). | Chunk-5 play-loop architecture notes (§5); ADR-005, I-2; SPEC-012 | ✅ 2026-06-17 — design-intent amendment (precedent D-3: declares a boundary ahead of the build; NOT an engine-behavior claim, so not gated by D-9) |
...
| D-11 | **Record/derive discipline.** Record *independent* facts directly; for *coupled* facts record the generating structure and derive the rest, so coupled facts are coherent by construction. Durations are recorded (independent — `fn_move_duration`, ADR-036); spatial distances are derived from coordinates (coupled, SPEC-018). | Chunk-5 play-loop architecture notes (§11); ADR-036; SPEC-018 | ✅ 2026-06-17 — design-intent amendment (precedent D-3: declares a boundary ahead of the build); record-side now realized in code per ADR-036 (D-9) |
| D-12 | **Spatial is a bounded subsystem — its own engine.** It owns geometry only: positions, and derived distance / route-time. It never owns world-truth beyond geometry. **Distance ≠ reachability** — distance (how far) is geometry; whether one can go at all (a wall, a locked door, no path) stays with move-validity (SPEC-017). Boundary declared ahead of build (precedent: D-3 declared the Image-Platform boundary pre-build). | Chunk-5 play-loop architecture notes (§12); SPEC-017/018 | ✅ 2026-06-17 — design-intent amendment (precedent D-3: declares a boundary ahead of the build; NOT an engine-behavior claim, so not gated by D-9) |
| D-13 | **Model-agnostic per-seat LLM routing**  Every play-loop LLM call runs through a per-seat driver, bound by config and freely swappable, with no provider SDK in the Core; a seat binds a model only if the driver satisfies that seat's declared capabilities (canon-emitting seats — decompose, resolve — require constrained-decoding of the closed event vocabulary). The quarantine (perception-bound, propose-only, never canon authority) holds per seat regardless of bound model — model choice affects output quality, never canon integrity or perception isolation. Provider SDKs and model drivers live *only* in the bridge layer (`core/api`), never in the canon engine (`core/db`); seats bind by *reported* capability — fail closed. | ADR-P018; doc 07; cf. ADR-009, ADR-020, D-3, D-10 | ✅ 2026-06-18 — Accepted at Leg-2 gate (D-9: established by ADR-P018; `core/api` bridge + `bridge_test.go` green, no provider SDK in `core/db`) |
