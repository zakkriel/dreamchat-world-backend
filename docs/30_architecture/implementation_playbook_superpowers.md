...
| **5** | **Fast-path play loop (engine Phase 1)** 🪜Q3 | Deterministic action handlers, perception fan-out, entity registry, context assembler v1, narrator (read-only LLM), minimal beat input + streaming | **Play the Mara slice yourself, end-to-end, in the UI.** Then delete the transcript and verify the world lost nothing | Engine Phase 1 gate (Mara slice, scripted driver) + Bridge S1 gate (human driver). _(Shipped 2026-06-18 as a MOVE+SAY thin slice. The broader event vocabulary — ObjectRelocated, AttributeChanged, etc. — and the full context assembler (doc 06; dirty ladder, scoring, token budget) are deferred via SPEC-012/013/014/016/017/018 and ADR-014. Q3 was answered at the founder-run operator gate: data-layer no-leak (B-1/I-3) + replay invariance (I-1) + canon-authority (D-1) + partial-beat correctness, with the live model. The generation-layer multi-holder leak (O-1) remains a Phase-3 problem, stated explicitly. Gate tag: `chunk-5-play-loop-gate`.)_ |
