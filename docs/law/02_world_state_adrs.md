...
## ADR-035 — Perception about-ness is an explicit `perception_subject` junction, not a derivation from the source event's participants

**Status:** Accepted (chunk-3 gate, 2026-06-14)
**Decision:** Add a junction table `perception_subject(perception_id, entity_id)` to the Master DDL (doc 03 §1.3, alongside `perception_record`), populated **at write time** by the path that creates the perception. The subject(s) of a perception — the entities the belief is *about* — are recorded explicitly there. The existing derivation `perception_record.source_event_id → event_participant` is **retained as a fallback and as a validation cross-check**, not removed: for direct/witnessed perceptions the junction and the derivation must agree, and disagreement is a logged audit flag. A perception may have **multiple subject rows** (about-ness is a set of links, not a record split) because the epistemic attributes (`epistemic_type`, `confidence`, `distortion_level`, teller, decay) are *shared across the subjects of that one belief*. When per-subject epistemic attributes genuinely differ (e.g. confident about C, unsure about D in the same telling), that is **separate `perception_record` rows**, each with its own subject link — never one record carrying per-subject confidence.
**Rationale:** Deriving about-ness from the source event's participants is correct only while every perception is about entities that were *in* its event. It breaks on the **told-about-a-third-party** case — A tells B about C: the telling event's participants are A and B, but C (the subject of B's resulting `told` perception) is **not** a participant of the telling. The derivation yields {A, B} and silently loses C — in exactly the rumor / disclosure / gossip path the epistemic model exists to serve (B-1, B-7). doc 06's context assembler already depends on about-ness ("perceptions about entities present in the scene") with no column to read; it is unbroken today only because Phase 0A has no third-party tellings. The fix is the standard one across prior art: PROV separates an activity's *subject* from its agents (ISSUE-49 aboutness); RDF needs reification/RDF-star to state something *about* a third party; ActivityStreams carries an explicit `object` distinct from `actor`/`target`. The subject of an assertion is a first-class edge, not a back-derivation from participants.
**Consequences:** A new junction table is added to the frozen Master DDL (doc 03 §1.3); per the engine's freeze governance this addition is routed as a proposed ADR rather than an ad-hoc migration. No existing engine ADR, invariant (I-1…I-10), or DDL column is modified or superseded — this is purely additive; the derivation path stays as fallback/validation. Touches I-2 (provenance) positively: about-ness becomes explicit lineage rather than an inference. The migration, write-time population (telling/extraction path supplies the subject), pgTAP coverage (positive/negative + a derivation-agreement guard), and the read-side use in doc 06 assembly are **owned by the Chunk 3 plan**; this ADR records the decision and the schema shape, not the migration. On acceptance under the Chunk 3 gate, doc 03 §1.3 gains the `perception_subject` DDL and doc 06's about-ness inclusion reads the junction (with the source-event derivation as fallback).
...
**Provenance:** Surfaced by the external canon/perception pressure-test (told-about-a-third-party failure mode) and filed as SPEC-008 in `docs/open-spec-items.md`; drafted here as a proposed engine ADR because `perception_record` lives in the frozen Master DDL (D-5: schema additions to the frozen set are ADR-gated, not code workarounds).

## ADR-036 — The World Clock advances by committed-event durations (action-driven time)

**Status:** Accepted (2026-06-18; Chunk-5 Leg-1 gate). Evidence (D-9): `apply_beat` (migration `20260618090001`) advances the World Clock by committed-prefix durations; pgTAP `95` proves clock placement in distinct `(in_world_tick, beat_seq)` slots, `96` proves an interrupted chain advances by the prefix's durations only (`ticks_advanced` = the move's 5, not the halted say's 0), `97` proves the durations/perceptions are deterministic on replay of the same beat+state, and `replay_0A()` returns true after a generated beat — all green in this chunk's execution.
**Decision:** Within a beat, in-world time advances by the **sum of the durations of the events that actually commit** — not per beat, not by wall-clock. Each committed event is assigned a duration by the **engine** from recorded deterministic world data (`fn_move_duration`; SAY ≈ 0), **never** by the LLM. An interrupted chain advances the clock by the **committed prefix's** durations only. `(in_world_tick, beat_seq)` are assigned by `apply_beat` and remain UNIQUE per world over accepted events (ADR-034, `uq_ce_accepted_order`); a zero-duration event shares the prior tick and increments `beat_seq`.
**Rationale:** Wall-clock dies on Q1/I-1 (replaying the log at a different real time → different clock); per-beat dies on C-6 (every Continue would advance world-time). Action-driven time survives both: pure Continue commits no events → no time passes (C-6); replay reproduces the clock because it reproduces the events and their recorded durations (Q1/I-1). Partial-beat time *requires* per-action time (§9–§10 are one decision). Durations are independent facts → recorded directly (D-11); the spatial-derive (coupled distance) is deferred (SPEC-018) and has no consumer in a model-free, hand-authored-places slice.
**Consequences:** Extends ADR-021/030 (which left 0A "assigning ticks by hand"): the beat loop now assigns and advances the clock. Durations live in engine-side data/functions, never in model output (the §10 Q1 guardrail). Supersedes nothing; adds the advancement rule the play loop assumes.

## ADR-037 — An unwitnessed event is canon, and `indirect` is how it becomes knowable

**Status:** Accepted (founder ruling, 2026-09-04). Additive; supersedes nothing. Evidence (**D-9**): migration `20260904230000_epistemic_indirect.sql`; the removed refusal and its two corrected belt comments in `core/api/worldgenesis.go`; `core/api/unwitnessed_test.go` (an unwitnessed event validates, the player floor still refuses, `indirect` writes with `acquired_tick > valid_tick`, and the closed set stays closed).
**Decision:** Two changes, one small and one that only reads small. **(1)** The genesis belt no longer refuses an authored event whose knowledge list is empty. **This reverses no decision** — `ADR-005` already decided it in the sentence *"One canon event fans out to **zero-to-N** perceptions"*, and the belt contradicted its own founding ADR while its doc comment listed *"a secret held by nobody"* among things "the world cannot survive". `SPEC-040` recorded that the reversal needed a superseding ADR; that was wrong, and this ADR exists for change (2), not for permission the engine already had. The player floor is untouched and still refuses: *"nobody knows this"* and *"the player knows this"* are different claims. **(2)** `epistemic_type` gains **`indirect`** — knowledge perceived **through a medium**: a recording, a spell, a dream, a scrying. It is distinct from `told` (a person's *mind* stands between you and the event) and from `inference` (your own *reasoning* does); a garage camera's tape is neither, being the event itself displaced in time. Reliability is **not** this word's job: `perception_record` already carries `confidence` and `distortion_level`, so a dream is `indirect` at low confidence and a tape is `indirect` at high. The type says how knowledge arrived; the numbers say how far it is trusted.
**Rationale:** The founder's case is physical, not epistemic: *"a car getting on fire and blowing up alone… it can happen and nobody is there to watch it. it destroys an item so a canon event needs to be there."* State changed, so canon must record it or replay breaks (**I-1**), and there is no secret and nobody keeping one. An earlier draft of this design proposed that every unwitnessed event must have a *carrier* to be legitimate; the car defeats that rule outright, and it was dropped. What the carrier idea was actually right about is the **reveal**, and the founder placed it correctly: the medium is optional, and it is what makes an unwitnessed event knowable later. The mechanism needs no second door into canon, because **nothing but a perception ever links to canon** — `perception_record_source_event_id_fkey` is the only such reference in the schema and it is `NOT NULL`. Founder: *"NOTHING ever links to cannon but perceptions. if a recording shows something. it shows the perception."* So the tape does not point at the event; it **holds a perception of it**. That is already legal: `holder_id` carries no foreign key and no kind check, exactly as the "Common Knowledge" `faction` pseudo-entity already exploits, and `fn_visible_perceptions` returns only perceptions held by the viewer or by a faction/group — so an artifact's perception is invisible to everyone until something grants them their own, which is precisely right for a sealed tape.
**Consequences:** A world may now author history nobody witnessed, which the fill could not previously express and which makes an authored past honest rather than uniformly observed. `SPEC-050` measured canon as *not scaling with depth* (12 events for 35 locations at depth 3 against 4–12 for 9–11 locations at depth 1); requiring a holder made every event more expensive to author and narrower in what it could say, so removing that requirement should make events cheaper, not merely more truthful. **Nothing is built for the reveal yet** and this ADR does not pretend otherwise: `generate_perceptions` derives holders from `event_participant` and hardcodes `acquired_tick = valid_tick = the event's tick`, so it cannot yet grant a late perception to someone who was not present; `provenance_edge` — whose `source_kind` already admits `'perception'` and whose `how_type` already contains `witnessed_by`, `reported_by` and `inferred_from` — is declared and **entirely dormant, written by nothing**; and no action exists for watching a tape or casting a divination. Those are `SPEC-040`'s remaining work, and building the minting path before an action that calls it would be dead code. The `indirect` addition is strictly additive: no existing value changes meaning and every perception written before the migration keeps its type. Routed as an engine ADR per **D-5** because `perception_record` lives in the frozen Master DDL (doc 03 §1.3), the same path `ADR-035` took.
**Provenance:** Raised as `SPEC-040` (founder, 2026-08-28: *"a Faction might have canon that is not public or not known by anybody… if we don't consider that we have a very shallow world"*), and settled 2026-09-04 across a sparring session in which two of the reviewing agent's own framings were rejected by the founder and replaced: the carrier-as-requirement, and an artifact→event pointer that would have opened a second door into canon. The `indirect` word is the founder's: *"like direct but perceived through another medium (a recording, a spell, a dream, etc)"*.

## ADR-038 — Heard words, described owners, and recognized people are different knowledge

**Status:** Accepted (founder ruling, 2026-09-06). Implemented locally on `engine/shared-speech-perception`; not deployed. The final heard-first prompt and explicit name-association entry rule passed all nine saved live scenes using application-built requests on the unchanged production Flash binding; the real Go decoder accepted all nine responses. Coverage: ordinary/ruled introductions, ambiguous names, descriptions of absent people, distraction, and one receiver-specific utterance. This is one successful response per scene, not a reliability or whole-turn affordability measurement. Provider overloads required delayed diagnostic retries; no runtime retry or model upgrade was added. Supersedes SPEC-033's canonical-name scanning as a teaching rule and SPEC-038's numeric attention design for speech. Other event types and the module runtime are outside this change.

**Decision:** Ordinary and ruled `Communicated` events use one core perception application path. Code supplies co-present listener candidates, each listener's sourced knowledge and known references, recorded activities, and existing physical computations. The existing resolve model judges attention and name associations from those facts. Code validates the result's structure, candidate coverage, references, and incompatible fields; it does not imitate understanding with canonical-name matching, quotation-based proof, attention scores, or prose grading. No model work runs on reads or once per name.

Each listener's accepted result records attention (`abstain` or an attentional block with a reason), heard words, and name associations. A missing required result is an error, never successful no-learning. The model cannot invent player inattention, impersonate a physical restriction, or override one. Hidden ruled speech creates no perceptions, including for the speaker, and requires no perception model call.

A heard name word alone does not identify an actor. Each association's owner carries required nullable `actor_id` and `description`, plus required `about_actor_ids`; there is no exclusive owner category. At least recognition or a meaningful description must exist, and related actor references require a description. A non-null `actor_id` writes sourced `name_knowledge` and a subject link, making that person available to the holder's actor-specific reads. A description is retained in sourced knowledge even when the same association recognizes an actor. Related actor IDs add subject links only: they are never promoted to the name's owner. A description without recognition creates no actor or identity knowledge. Neither recognition nor description changes canonical identity or guarantees the speaker told the truth. The existing first-association behavior remains; aliases and name correction are not added.

Add nullable `perception_record.spoken` to the Master DDL's perception record: the words this holder perceived, not a join back to the source's canonical utterance. The canonical payload retains both the source words and the accepted judgment for replay. Account text passes through the identity-based `fn_viewer_text` before heard words are appended. The boundary's lexical guard may preserve words present in visible stored speech; `fn_display_name` remains identity-based. Narration quotes come only from the holder's perceived speech, never a canonical fallback. This lexical check does not prove a model used a heard word with the right meaning.

**Interruption order:** Keep the existing pre-action interruption opportunity, but NPC cognition receives only speaking cues before speech commits, never unsaid words or hidden references. After a successful speech commit, a separate cognition round reads actual holder-specific perceptions. The founder explicitly accepted this extra model round. Post-reaction cognition likewise reads committed outcomes, not the original interrupted attempt. Keep the existing reaction-depth limit; no recursive dialogue cascade.

**Transaction order:** Earlier events in a ruling are visible to later speech judgments. Consumed held intentions become resolved at the start of that transaction, with rollback restoring them. Ruling-wide attribute writes and mints retain their existing position after the event list. Canon, perceptions, name associations, and bookkeeping commit together.

**Rationale:** The former ordinary path scanned words against canonical names, while the ruled path did not teach at all. The same utterance therefore changed knowledge differently depending on its commit door, and a matching word could silently become recognition. The founder chose the existing physics division: code computes facts, the model interprets them, core records consequences. Perception remains core; future modules contribute their defined mechanics rather than separate perception engines.

**Release:** This is an incompatible speech-writer cutover. Pause writes, apply the migration, start the matching binary, verify speech, then resume writes. Do not migrate while an old binary serves speech; binary-only rollback is unsafe. Historical backfill may copy source words only when the complete utterance already appears exactly in the holder's stored content or as a complete quoted utterance. Unrecoverable receiver variants stay unset; existing perceptions and learned names remain. No production release is authorized by this implementation round.

**Provenance and scope:** Founder-approved shared speech facts/result contract and subsequent explicit interruption ruling; D-1, B-1/B-2/B-4/B-6/B-7, ADR-005/009/035, and ADR-P025. This additive perception column follows the frozen-schema amendment route used by ADR-035/037. No hearing radius, acoustic rule, physical override mechanism, world-creation change, image-platform change, or generic plugin framework is introduced.
