...
**ADRs:** ADR-001 onward in `docs/law/02_world_state_adrs.md`. The load-bearing ones for everything downstream:
...
| D-14 | **FE rendering model — one engine, a catalog of kinds.** The FE carries **one rendering path per job** — never a hardcoded path *and* a generic path for the same data. The backend tags data by **kind** (it already does: `epistemic_type`, group keys, participants, carrying) and sends **only data + a kind tag, never layout or look**; the FE owns a **catalog** mapping each kind to a crafted component plus all presentation/craft. **Known-structure UI** (every chunk-6 surface — scene, narration, input, participants, Aux Current/Known, carrying) renders from native catalog components; **unknown-structure UI** — 3rd-party **module** widgets whose shape the FE cannot know ahead of time — renders through a generic fragment renderer **deferred to S4**, built when the first module needs it. Catalog components may carry backend-generated **actions** (suggested affordances): clicking one **submits a beat**, identical to typing the action — an accelerator that never replaces the free-text input and submits intent only, never deciding outcomes (D-7). Aux is **push + pull** (reacts to user focus/clicks *and* to world events via the beat stream's `aux delta` frames). Modules compose from the catalog into named slots (D-2); the FE never hosts module *code* and the render contract stays **declarative-only**, so 3rd-party code is never required in the renderer (trust model — declarative now, WASM/server sandbox if code ever arrives — deferred to the module-architecture doc). | FE Architecture working session 2026-06-19 (chunk-6 pre-brainstorm); ADR-P019 | ✅ 2026-06-19 — design-intent amendment (precedent D-3: declares an FE boundary ahead of the build; FE presentation-only per D-7, NOT an engine-behavior claim, so not gated by D-9). Attaches to D-2, D-7, GA-3. |
| D-15 | **Theme architecture — neutral skeleton + swappable skins.** The FE is a **neutral skeleton of named slots**; a theme changes **visuals only** — never structure, behavior, features, or requirements. Themes may go **deep** (swap a component's visual *form*, not merely recolor: a knowledge item is a gold card in Fantasy, a torn typewriter strip in Horror, a HUD readout in Sci-fi — same data, slot, behavior). **Cost discipline:** *tokens are the floor* (a theme supplying only colors/fonts gets a coherent recolor for free); *per-component art direction is opt-in* (premium themes go deep where they choose — no promise of infinite cheap deep themes). **Two independent atmosphere layers:** *world art* (scene/portrait/item imagery) is **content** bound to the world (Image Platform — D-3, D-8); *chrome theme* (palette, type, ornament, frames, component variants) is a **swappable skin** the world supplies as a default and the **user may override**. World theme data carries `schema_version` + runtime validation (D-4). **Genre-agnostic (GA-3):** the system never knows "fantasy" — it reads a world's accent/art/tokens as plain data; no hardcoded genre modes. Fantasy is theme #1 (built first because the mockups exist, to prove the theming architecture) — **not** the baseline/default. | FE Architecture working session 2026-06-19 (chunk-6 pre-brainstorm); ADR-P019 | ✅ 2026-06-19 — design-intent amendment (precedent D-3: declares an FE boundary ahead of the build, NOT an engine-behavior claim, so not gated by D-9). Reinforces GA-3; cites D-3, D-4, D-8. |
...
| ID | Gap |
|---|---|
| G-1 | ✅ **Resolved** (resolution artifact — the World Graph Inspector spec, formerly `30_architecture/world_graph_inspector_debug_view.md` — lost in the 2026-08-27 consolidation; needs a ruling on where it lives now): minimal read-only **World Graph Inspector** — provenance up/downstream + bundles from Phase 4, creator/debug only, no engine changes. |
| G-2 | **PRD→DDL mapping appendix** — sequenced: written after all Compendium PRDs are validated. |
| G-3 | ✅ **Resolved:** `docs/design/mvp_slice_and_bridge.md` — slices S0–S4 vs engine phases, projection/beat API contract, module UI slots, staged image pipeline. |
| G-4 | ✅ **Resolved:** engine `epistemic_type` is canonical. Mapping doc `docs/design/01_epistemic_type_canonical_enum.md` created; all PRD `source_type` enums patched to reference it. |
...
3. The register becomes the standing law of the docs (this file, now at `docs/law/06_rules_register.md`), and later the seed for CI checks (Part A already is).
