...
func TestAuthorKickstartOffersThree(t *testing.T) {
	doc, _, err := authorWorld(context.Background(), NewFakeWorldUnderstandingDriver(), NewFakeWorldFillDriver(), NewFakeWorldFillReviewDriver(), "a harbour town at closing time", nil, nil, nil, 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAuthorKickstartGroundsCustomOpening(t *testing.T) {
	doc, _, _ := authorWorld(context.Background(), NewFakeWorldUnderstandingDriver(), NewFakeWorldFillDriver(), NewFakeWorldFillReviewDriver(), "a harbour town at closing time", nil, nil, nil, 0)
	kseat := NewFakeWorldKickstartDriver()
