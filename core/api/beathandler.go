...
type beatMessage struct {
	...
	// non-null exactly for kind "speech" (beat_frame/5, narration/3). Split so the frontend can format
...
func narrateMessages(segments []NarrationSegment, labelFor map[string]string) ([]beatMessage, string) {
	...
	for _, s := range segments {
		m := beatMessage{SpeakerID: s.SpeakerID, Kind: s.Kind, Text: s.Text, Quote: s.Quote, Emotion: s.Emotion}
		if s.SpeakerID != nil {
