...
- `resolve.txt` — the resolve (referee) seat. Injected as the STABLE HEADER (prompt-prefix) at the
  ...
  sections.
- `speech_perception.txt` — a specialized task for the existing resolve driver, not a new seat.
  `buildSpeechPerceptionPrompt` supplies final speech and holder-specific database facts; the
  result distinguishes attention, heard words, and name ownership (`ADR-038`).
