...
// Movement binds a portal artifact in the room. This is the only travel a beat can express today
// (remote locations are never candidates), so it is the move that has to work.
func TestFakeIntent_MovementBindsPortalArtifact(t *testing.T) {
