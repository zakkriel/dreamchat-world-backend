...
var allSeatNames = []string{
	...
	SeatWorldGenesis.Name, SeatWorldUnderstanding.Name, SeatWorldFill.Name, SeatWorldFillReview.Name, SeatWorldInterview.Name, SeatWorldKickstart.Name,
}
...
func requestTimeoutFor(seat, provider string, lookup func(string) string) string {
	...
	if timeout == "" && (seat == SeatWorldGenesis.Name || seat == SeatWorldUnderstanding.Name || seat == SeatWorldFill.Name || seat == SeatWorldFillReview.Name) {
		return "300"
	}
	return timeout
}
