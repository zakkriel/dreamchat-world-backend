...
SELECT is( (SELECT count(*) FROM (
       ...
       0, 'DETERMINISTIC-DOMAIN-ORDER: (world_id,in_world_tick,beat_seq) unique across accepted events');
SELECT is( (SELECT count(*) FROM (
       ...
       0, 'DETERMINISTIC-DOMAIN-ORDER: mutation order key unique per (entity,attribute)');
