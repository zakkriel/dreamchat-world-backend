...
-- SAME move beat is run TWICE from IDENTICAL co-presence state — Player@tavern-uuid, Jonas@vault-uuid —
-- with a FRESH SETUP between the runs (Player is re-placed at tavern-uuid before run 2). It is NOT two
-- calls on accumulated state (the test-96 trap): run 1 leaves Player at vault-uuid, so run 2 MUST
-- re-place them. vault-uuid is a uuid NO seed/noise actor ever occupies, so co-presence at the
-- destination is exactly {Jonas} regardless of the seed's noise positions (seed-independent, not
-- fragile). A MOVE is used (not a say) precisely so the discovery perception's about-subject
-- (perception_subject) is exercised. The compared set keeps EVERY semantic field — holder_id,
-- epistemic_type, content, the about-subject, and the WITHIN-BEAT relative tick (acquired_tick -
-- start_tick, which is deterministic) — and strips ONLY the non-semantic volatiles: perception_id
-- and the absolute start band (deliberately varied).

INSERT INTO entity_registry (entity_id, world_id, entity_kind, canonical_name) VALUES
 ('e5ffffff-0000-0000-0000-000000000001','11111111-1111-1111-1111-111111111111','location','test-vault-97'),
 ('e5ffffff-0000-0000-0000-000000000010','11111111-1111-1111-1111-111111111111','location','test-tavern-97');

-- Jonas → vault-uuid (stays put for both runs; vault-uuid is off the noise map)
...
INSERT INTO state_mutation (world_id, event_id, entity_id, entity_kind, attribute_path, new_value, valid_from_tick, valid_from_seq)
VALUES ('11111111-1111-1111-1111-111111111111','e5000000-0000-0000-0000-000000000081','aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa','actor','attrs.location_id', to_jsonb('e5ffffff-0000-0000-0000-000000000010'::text),1210,0);
SELECT apply_beat('11111111-1111-1111-1111-111111111111','aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
  '[{"type":"move","to":"e5ffffff-0000-0000-0000-000000000001"}]'::jsonb, 1300, 100, 'fast_path');

-- FRESH SETUP between runs — re-place Player at tavern-uuid (run 1 left them at vault-uuid); Jonas
-- stays at vault-uuid. Run 2's starting co-presence is now identical to run 1's.
INSERT INTO canon_event (event_id, world_id, event_type, summary, in_world_tick, beat_seq,
                         status, accepted_at, visibility_scope, origin)
...
INSERT INTO state_mutation (world_id, event_id, entity_id, entity_kind, attribute_path, new_value, valid_from_tick, valid_from_seq)
VALUES ('11111111-1111-1111-1111-111111111111','e5000000-0000-0000-0000-000000000082','aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa','actor','attrs.location_id', to_jsonb('e5ffffff-0000-0000-0000-000000000010'::text),1320,0);
SELECT apply_beat('11111111-1111-1111-1111-111111111111','aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
  '[{"type":"move","to":"e5ffffff-0000-0000-0000-000000000001"}]'::jsonb, 1350, 100, 'fast_path');
